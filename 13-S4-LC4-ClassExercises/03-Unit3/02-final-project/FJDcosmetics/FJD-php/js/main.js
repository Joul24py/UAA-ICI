...
function actualizarCarritoUI(){
    ...
    fetch('http://localhost/Pruebacarrito/api/carrito/api-carrito.php?action=mostrar')
    .then(response =>{
    ...
    .then(data =>{
        ...
        data.items.forEach(element => {
            html += `
                <div class='fila'>
                    <div class='imagenes'><img src=imagenes/${element.imagenes}' width='100' /></div>
                    <div class='info'>
...
const addItemToCarrito = id =>{
    ...
    fetch('http://localhost/Pruebacarrito/api/carrito/api-carrito.php?action=add&id=' + id)
    .then(response =>{
...
const removeItemFromCarrito = id =>{
    ...
    fetch('http://localhost/Pruebacarrito/api/carrito/api-carrito.php?action=remove&id=' + id)
    .then(res =>{
        return res.json();
    })
    .then(data =>{
