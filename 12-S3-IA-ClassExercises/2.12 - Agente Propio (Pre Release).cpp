...
main()
{
	...
	do
	{
		...
		do
		{
			...
			if(intel==0) //Número aleatorio para agente tonto
			{
				random=1+rand()%(5-1);
			}
			else //Número aleatorio para agente inteligente
			{
				random=aleatorio();
			}
		...
		//Aumenta la cuenta de agentes tontos o comienza la de inteligentes------------------------------------
		if((menucont1+1==menucant1) && (intel==0))
		{
			intel=1;
		}
		menucont1++;
